...
П.С. В `docker-compose.yml` вам нужно будет сменить порт `8001` на `80`.
У меня так настроено, потому что на одном сервере и IP висит несколько несколько сайтов.
***
